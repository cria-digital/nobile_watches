...
export function BrandPageClient({ brandName }: BrandPageClientProps) {
  ...
  // Detecta se é a visualização de todos os produtos
  ...
  const filterOptions = getFilterOptions(brandProducts);
  ...
  const {
    ...
    initialFilters: isAllProducts
      ? {} // Sem filtro inicial quando for "all"
      : { brands: [brandName] }, // Filtro de marca quando for específico
  });
  ...
  const handleApplyFilters = (newFilters: AppliedFilters) => {
    if (isAllProducts) {
      // Quando for "all", permite filtrar por qualquer marca
      setAllFilters(newFilters);
    } else {
      // Quando for marca específica, preserva a marca da página
      setAllFilters({
        ...newFilters,
        brands: [brandName],
      });
  ...
  if (isError) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] text-center px-6">
        ...
        <p className="text-gray-500 dark:text-gray-400">Tente novamente mais tarde.</p>
      </div>
    );
  }

  // Define título e breadcrumb baseado no contexto
  const pageTitle = isAllProducts ? "Todos os Relógios" : brandName;
  const breadcrumbLabel = isAllProducts ? "Todos os Relógios" : brandName;
          ...
          <div className="mb-6 lg:mb-8">
            ...
            <div className="flex lg:h-[44px] items-center justify-between">
              <div className="flex-1">
                <h1 className="text-3xl lg:text-[32px]">{pageTitle}</h1>
              </div>
            ...
            {/* Ações rapidas - Desktop - Apenas para usuários autenticados */}
            {isAuthenticated && (
